  ...
  export default {
    ...
    methods: {
      ...
      loadTableData (pageNo) {
        ...
        this.$http.get('/base/eric/users', self.searchFormModel)
      ...
      editTableRowClick (index, row) {
        this.$router.push('/Main/EricUserEdit/' + row.id)
      },
      // tablb 表格删除行
      deleteTableRowClick (index, row) {
        ...
        this.$confirm('确定要删除吗, 是否继续?', '提示', {
          ...
          this.$http.delete('/base/eric/user/' + row.id)
            .then(function (response) {
      ...
      addTableRowClick () {
        loadDataControl.add(this.$store, 'EricUserAddLoadData=true')
        this.$router.push('/Main/EricUserAdd')
      }
    },
    watch: {
    }
